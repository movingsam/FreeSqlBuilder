...
export class NewTemplateComponent implements OnInit {
  ...
  ngOnInit() {
    this.client.get<RootPath>('http://localhost:5000/api/RootPath')
      .subscribe((data) => {
  ...
  AddTemplate(): void {
    ...
    this.client.post<ImportData>(`http://localhost:5000/api/Template`, res)
      .subscribe((data) => {
        console.log(data);
  ...
  import(): void {
    if (this.value.endsWith('.cshtml')) {
      this.client.get<ImportData>(`http://localhost:5000/api/Cshtml/Import?path=${this.value}`)
        .subscribe((data) => {
          console.log(data);
          this.code = data.fileContent;
        });
